...
t 5 'XDG_CONFIG_HOME fallback (no -c)'
xdg="$WORK/xdg"
mkdir -p "$xdg/pathset"
...
t 47 '-k man reads ~/.config/pathset/man'
...
t 48 '-k info reads ~/.config/pathset/info'
...
t 49 '-k fpath reads ~/.config/pathset/fpath'
...
t 50 '-k path is equivalent to no -k'
home_p="$WORK/home_p"
...
t 51 '-k bogus exits 2 with error'
...
t 52 '-k with no argument exits 2'
...
t 53 '-c CONFIG -k man reads CONFIG (kind ignored when -c given)'
...
t 54 'legacy ~/.pathset/<kind> fallback works for non-default kind'
...
cfg_ev="$WORK/cfg_ev"
printf '%s\n$MTVAR/usr/bin\n' "$A" >"$cfg_ev"
got_ev="$(MTVAR="" "$BIN" -c "$cfg_ev" 2>"$WORK/err_ev")"
rc_ev=$?
...
cfg_ts="$WORK/cfg_ts"
printf '%s\n%s/\n' "$A" "$A" >"$cfg_ts"
got_ts="$("$BIN" -c "$cfg_ts" -d 2>/dev/null)"
if [[ "$got_ts" == "$A" ]]; then
...
printf '%s/\n%s\n' "$A" "$A" >"$cfg_ts2"
got_ts2="$("$BIN" -c "$cfg_ts2" -d 2>/dev/null)"
if [[ "$got_ts2" == "$A/" ]]; then
...
got_ts3="$("$BIN" -c "$cfg_ts" 2>/dev/null)"
if [[ "$got_ts3" == "$A:$A/" ]]; then
...
printf '%s\n%sx\n' "$A" "$A" >"$cfg_ts4"
got_ts4="$("$BIN" -c "$cfg_ts4" -d 2>/dev/null)"
...
"$BIN" -c "$cfg1" -q >/dev/null 2>"$WORK/err_qc"
...
got_ae2="$("$BIN" -c "$cfg_ae" -q --allow-empty 2>/dev/null)"
rc_ae2=$?
...
"$BIN" -c "$cfg2" --allow-empty >/dev/null 2>"$WORK/err_ae3"
rc_ae3=$?
...
got_ck="$("$BIN" -c "$cfg1" --check 2>/dev/null)"
rc_ck=$?
...
"$BIN" -c "$WORK/does-not-exist" --check >/dev/null 2>/dev/null
rc_ck3=$?
if [[ $rc_ck3 -eq 1 ]]; then
...
cfg_co="$WORK/cfg_co"
printf '%s\n%s/co:lon\n' "$A" "$WORK" >"$cfg_co"
got_co="$("$BIN" -c "$cfg_co" 2>"$WORK/err_co")"
rc_co=$?
...
printf '%s\n$COLONVAR\n' "$A" >"$cfg_co3"
got_co3="$(COLONVAR="$WORK/co:lon" "$BIN" -c "$cfg_co3" 2>/dev/null)"
rc_co3=$?
